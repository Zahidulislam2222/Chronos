...
Chronos is a public portfolio demonstration of a watch storefront, not a retailer. Its products are fictional, its prices and specifications are illustrative, and the deployed preview does not create accounts, submit contact messages, accept orders or process payments. This distinction determines what can responsibly be implemented now. The review concerns the public static frontend and its release configuration; it does not certify the retained WordPress commerce backend for sale of real goods.
...
The deployed design is a static asset service. It has no public application database, no upload handler, no payment endpoint and no server-side execution of frontend business code. Production mounts contain the built release and web-server configuration, not source credentials or the WordPress tree. This removes several common attack paths, but does not eliminate browser, supply-chain, CDN-account, host or denial-of-service risks.
...
A public static website still processes network data. Cloudflare and the hosting provider participate in delivering requests and protecting infrastructure. IP addresses, request metadata and diagnostic records can matter even where the application has no customer database. The privacy page therefore must not say that no data reaches any server or that hosting is anonymous. Routine storefront access logging is disabled in the new configuration, and container diagnostic logs are size-bounded; provider and shared-host processing remains separate.
